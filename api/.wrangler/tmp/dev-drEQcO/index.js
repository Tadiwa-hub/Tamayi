...
// .wrangler/tmp/bundle-qo5DYH/strip-cf-connecting-ip-header.js
...
app.post("/api/admin/login", async (c) => {
  ...
  if (attemptInfo.lockoutUntil > now) {
    const remainingSecs = Math.ceil((attemptInfo.lockoutUntil - now) / 1e3);
    return c.json({
  ...
  if (password !== adminPassword) {
    ...
    if (attemptInfo.count >= 3) {
      attemptInfo.lockoutUntil = now + 10 * 60 * 1e3;
...
// .wrangler/tmp/bundle-qo5DYH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-qo5DYH/middleware-loader.entry.ts
